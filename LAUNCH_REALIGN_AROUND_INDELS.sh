...
# EXAMPLE USAGE:
# $HOME/Scratch/git_single_cell_sequencing_scripts/LAUNCH_REALIGN_AROUND_INDELS.sh $HOME/Scratch/TNBC_data/SraRunTable_TNBC_v2.txt $HOME/Scratch/TNBC_data TNBC
...
do
	...
	p3="$(echo $meta_prefix$three/$six/$f/$f$realinged |  awk -F"//" '{print $2}')"	
        ...
        dedup_bam=$(find $data_folder/$p1 -type f | grep '\b.RG.bam$')
	...
	realigned_bam=$(find $data_folder/$p3 -type f)
        ...
        if [[ ! $realigned_bam = *[!\ ]* ]]
                ...
                # IF dedup.bam EXISTS THEN PRINT THE PREFIX AND THE INPATH AND LAUNCH THE REALIGNMENT
		...
		echo "indel vcf "$indel_vcf
                ...
                qsub $HOME/Scratch/git_single_cell_sequencing_scripts/REALIGN_AROUND_INDELS.sh $prefix $dedup_bam $indel_vcf $inpath $subgroup
                fi
        # IF .realigned.bam ALREADY EXISTS THEN ls -lt
        else ls -lt $realigned_bam;
        echo "realignment already done, checking integrity..."
	samtools view $realigned_bam | tail -1
        fi
